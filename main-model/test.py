...
def main() -> None:
    ...
    for index in range(total_files):
        ...
        # clac the bicubic error
        bicubic_psnr_metrics += psnr(lr_y_tensor, hr_y_tensor).item()
        bicubic_ssim_metrics += ssim(lr_y_tensor, hr_y_tensor).item()
    ...
    avg_bicubic_psnr = 100 if bicubic_psnr_metrics / total_files > 100 else bicubic_psnr_metrics / total_files
    avg_bicubic_ssim = 1 if bicubic_ssim_metrics / total_files > 1 else bicubic_ssim_metrics / total_files

    print(f"Model PSNR: {avg_psnr:4.2f} dB, SSIM: {avg_ssim:4.4f}")
    print(f"Bicubic PSNR: {avg_bicubic_psnr:4.2f} dB, SSIM: {avg_bicubic_ssim:4.4f}")
