...
class TrainValidImageDataset(Dataset):
    ...
    def __getitem__(self, batch_index: int) -> [torch.Tensor, torch.Tensor]:
        ...
        if self.mode == "Train":
            ...
            hr_image = self.augmentation_transforms(hr_image)

            # Convert back to numpy array after augmentations
            hr_image = np.array(hr_image).astype(np.float32) / 255.
        ...
        # Convert to Y channel for processing
