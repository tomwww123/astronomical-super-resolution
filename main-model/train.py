...
import config
from dataset_with_augumentation import CUDAPrefetcher, TrainValidImageDataset, TestImageDataset
...
def main() -> None:
    ...
    if config.resume:
        ...
        best_ssim = 0 # checkpoint["best_ssim"]
        ...
        for param_group in optimizer.param_groups:
            param_group['lr'] = config.model_lr
            param_group['weight_decay'] = config.model_weight_decay
...
def validate(model: nn.Module,
    ...
    with torch.no_grad():
        while batch_data is not None:
            ...
            with amp.autocast():
                lr_up = F.interpolate(lr, scale_factor=config.upscale_factor, mode='bicubic', align_corners=False)
                sr = model(lr_up)
